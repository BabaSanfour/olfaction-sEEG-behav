...
def compute_correct_trigs_and_rest(run,timings,keep_all_non_art,trial_types):
    
    if trial_types == trial_E_conds:
        
        start_odor = [ trial.first_inspiration_time + t_win_start_odor_encoding for trial in run.trials ]
        stop_odor = [ trial.first_inspiration_time + t_win_stop_odor_encoding for trial in run.trials]
        start_rest = [ trial.first_inspiration_time + t_win_start_rest_encoding for trial in run.trials]
        stop_rest = [ trial.first_inspiration_time + t_win_stop_rest_encoding for trial in run.trials]
    
    if trial_types == trial_R_conds:
        
        start_odor = [ trial.first_inspiration_time + t_win_start_odor_recall for trial in run.trials ]
        stop_odor = [ trial.first_inspiration_time + t_win_stop_odor_recall for trial in run.trials]
        start_rest = [ trial.time + t_win_start_rest_recall for trial in run.trials]
        stop_rest = [ trial.first_inspiration_time + t_win_stop_rest_recall for trial in run.trials]
    ...
    for i,odor_range in enumerate(zip(start_odor,stop_odor)):
        ...
        if np.sum(keep_all_non_art[sel_odor]) == np.sum(sel_odor == True) and np.sum(keep_all_non_art[sel_rest]) == np.sum(sel_rest == True):
            ...
            nb_removed_trigs = nb_removed_trigs + 1
        
    print len(correct_trigs),len(correct_rest),nb_removed_trigs
    
    return correct_trigs,correct_rest,nb_removed_trigs
...
def return_sess_correct_trigs_and_rest(run,sess_timings,sess_keep_all_non_art,trial_types):
        ...
        #for trial_type in ['hit']:
            ...
            ## odor + rest
            correct_trigs,correct_rest,nb_removed_trigs = compute_correct_trigs_and_rest(run,sess_timings,sess_keep_all_non_art,trial_types)
        ...
        return sess_correct_trigs,sess_correct_rest,nb_removed_trigs_by_trial
