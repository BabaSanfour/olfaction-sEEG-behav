...
def compute_correct_trigs_and_rest(run,timings,keep_all_non_art,trial_type = 'all'):
    ...
    if trial_type == 'all':
        ...
        start_odor = [ trial.recognition_time + t_win_start_odor for trial in run.trials ]
        stop_odor = [ trial.recognition_time + t_win_stop_odor for trial in run.trials]
        start_rest = [ trial.time + t_win_start_rest for trial in run.trials]
        stop_rest = [ trial.time + t_win_stop_rest for trial in run.trials]
        ...
        inspi_no_odor = np.array([time for time in inspi_times if time not in odor_times and time > 0])[1:-2]
        print('no odor times',inspi_no_odor.shape)

        start_odor = [inspi + t_win_start_odor for inspi in inspi_no_odor]
        stop_odor = [inspi + t_win_stop_odor for inspi in inspi_no_odor]

    else:
        #start_odor = [ (trial.first_inspiration_time + t_win_start_odor) for trial in run.trials if trial.odor_num == int(trial_type)]
        #stop_odor = [ (trial.first_inspiration_time + t_win_stop_odor) for trial in run.trials if trial.odor_num  == int(trial_type)]
        start_odor = [ trial.time + (trial.recognition_time + t_win_start_odor) for trial in run.trials if trial.odor_num == int(trial_type)]
        stop_odor = [ trial.time + (trial.recognition_time + t_win_stop_odor) for trial in run.trials if trial.odor_num  == int(trial_type)]
        #start_odor = [ trial.time + t_win_start_odor for trial in run.trials if trial.odor_num == int(trial_type)]
        #stop_odor = [ trial.time + t_win_stop_odor for trial in run.trials if trial.odor_num == int(trial_type)]
        stop_rest = [ trial.time + t_win_stop_rest for trial in run.trials if trial.odor_num == int(trial_type)]
        stop_rest = [ trial.time + t_win_stop_rest for trial in run.trials if trial.odor_num == int(trial_type)]

        #start_odor = [ trial.time + t_win_start_odor for trial in run.trials if trial.odor_num == int(trial_type)]
        #stop_odor = [ trial.time + t_win_stop_odor for trial in run.trials if trial.odor_num  == int(trial_type)]
        #start_rest = [ trial.time + t_win_start_rest for trial in run.trials if trial.odor_num == int(trial_type)]
        #stop_rest = [ trial.time + t_win_stop_rest for trial in run.trials if trial.odor_num == int(trial_type)]
    ...
    for i,odor_range in enumerate(zip(start_odor,stop_odor)):
        ...
        print 'non art', keep_all_non_art[sel_odor]
        correct_trigs.append(odor_range)

    print 'nb correct trig',len(correct_trigs),'removed',nb_removed_trigs
    print 'correct trigs', correct_trigs
    return correct_trigs, nb_removed_trigs
...
def return_sess_correct_trigs_and_rest(run,sess_timings,sess_keep_all_non_art,trial_types):
        ...
        #for trial_type in ['hit']:
            ...
            ## odor + rest
            correct_trigs,nb_removed_trigs = compute_correct_trigs_and_rest(run,sess_timings,sess_keep_all_non_art,trial_type = trial_type)
        ...
        return sess_correct_trigs,nb_removed_trigs_by_trial
...
def compute_subj_correct_bip_ampl(export_dir, f_start, f_stop, name = 'SEMC',exp = 'R',thr_nb_artefact = 120):
    ...
    for i,sess_index in enumerate(sess_indexes):
        ...
        print 'sess correct trigs', np.array(sess_correct_trigs).shape
