...
from export_inband_amplitude2 import return_electrode_names_and_coords,return_non_art,return_sess_correct_trigs_and_rest
...
def compute_subj_correct_sigs(export_dir, name = 'SEMC',exp = 'E',thr_nb_artefact = 120):
    ...
    for i,sess_index in enumerate(sess_indexes):
        ...
        sess_correct_trigs,sess_correct_rest,nb_removed_trigs_odor_by_trial, nb_removed_trigs_rest_by_trial = return_sess_correct_trigs_and_rest(run,list_sess_timings[i],list_keep_all_non_art[i],trial_types)
        ...
        with open(log_file,'a') as log:
            ...
            log.write("Number of removed artefact trigs Odor: \n%s\n\n"%(" ".join([": ".join(trial) for trial in zip(trial_types,map(str,nb_removed_trigs_odor_by_trial))])))
            log.write("Number of removed artefact trigs Rest: \n%s\n\n"%(" ".join([": ".join(trial) for trial in zip(trial_types,map(str,nb_removed_trigs_rest_by_trial))])))
