...
path_mask = join(path, 'Visbrain/All_subjects/allrois_3wins/masks_visbrain/')
path_to_save = join(path, 'Visbrain/All_subjects/allrois_3wins/Brain_Plots/')
npz_form = join(path_npz, '{}_sources_{}_{}_EpiScore_Expi_AAL_BA_{}.npz')
masks_vis_form = join(path_mask, '{}_mask_min{}_{}_win{}_th{}.npy')
f_form_save = join(path_to_save, '{}_signif_min{}_AUC_bsl_{}_{}_3wins_th{}.png')
###############################################################################
...
for freq, bsl, th, min_sig, method in product(freqs,bsls,ths,min_sigs, methods):
	...
	for win in wins:
		...
		xyz, data = arch_sig['s_xyz'], arch_sig['s_da'][:,win]
		s_obj_c = SourceObj('modulation', xyz, data=data,mask=mask)
		b_obj_top.project_sources(s_obj_c,'modulation', cmap='autumn', clim=(0.5,0.8),
			vmin=0.6, under='grey',radius=12.)
		...
		b_obj_bot.project_sources(s_obj_b,'modulation', cmap='autumn', clim=(0.5,0.8),
			vmin=0.6, under='grey',radius=12.)
		...
		b_obj_r.project_sources(s_obj_r,'modulation', cmap='autumn', clim=(0.5,0.8),
			vmin=0.6, under='grey',radius=12.)
		...
		b_obj_r2.project_sources(s_obj_r2,'modulation', cmap='autumn', clim=(0.5,0.8),
			vmin=0.6, under='grey',radius=12.)
		...
		b_obj_l.project_sources(s_obj_l,'modulation', cmap='autumn', clim=(0.5,0.8),
			vmin=0.6, under='grey',radius=12.)
		...
		b_obj_l2.project_sources(s_obj_l2,'modulation', cmap='autumn', clim=(0.5,0.8),
			vmin=0.6, under='grey',radius=12.)
