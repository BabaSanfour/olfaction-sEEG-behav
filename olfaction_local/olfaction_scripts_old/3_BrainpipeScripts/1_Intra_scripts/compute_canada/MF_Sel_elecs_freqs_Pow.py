...
def MF_Selfeat(su,ti):
    savename = "{}/{}_{}_all_MVPA_scores.npz".format('MF_resultsR',su,ti)
    ...
    if path.exists(savename):
	    ...
	    for power in pow_list:
	        dat = power[:,:,ti,:].swapaxes(0,-1)
	        pow_data_elec.append(dat)
	    subsampled_data = np.concatenate(pow_data_elec, axis=0)
	    labels = np.hstack([np.array([i]*pw.shape[0]) for i, pw in enumerate(pow_data_elec)])
...
if __name__ == "__main__":
    ...
    pow_list.append(mat0['xpow'][2:7,:,17:52,:])
    nelecs, nfreqs = mat0['xpow'].shape[1], range(2,7)
    mat1 = np.load(PATH+'dataR/'+su+'_odor_'+conds[1]+'_bipo_sel_physFT_pow.npz')
    pow_list.append(mat1['xpow'][2:7,:,17:52,:])

    Parallel(n_jobs=-1)(
        delayed(MF_Selfeat)(su,ti) for ti in range(35)
        )
