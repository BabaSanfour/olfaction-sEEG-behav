...
th, tresh, feat, corr = '0.01',0.01, 'powAUC', True
###############################################################################
...
methods = ['s_Mai_RL']
clim = (0.5,0.9)
thresh = np.min([0.61, 0.65, 0.56, 0.64, 0.55, 0.68]) #TPSim
...
for win,min_sig, method in product(wins,min_sigs, methods):
	...
	for i,freq in enumerate(freqs):
		...
		idx_ant = np.where([x[1]>-26 for x in arch_sig['s_xyz'][idx_rois]])
		xyz = arch_sig['s_xyz'][idx_rois][idx_ant]
		da = arch_sig['s_da'][idx_rois][idx_ant]#,10:30]#[idx_rois,:][0]
		print('da shape', da.shape)

		subjects = arch_sig['su_codes'][idx_rois][idx_ant]
		
		pow0, pow1 = arch_sig['s_elec_pow0'][idx_rois][idx_ant], arch_sig['s_elec_pow1'][idx_rois][idx_ant]
		print('pow shape',pow0.shape,pow1.shape)

		power = ((pow1-pow0)/abs(pow0))*100
		mask = np.load(masks_form.format(freq,str(win),th))[idx_rois][idx_ant]
		...
		roi_obj_f.project_sources (s_obj_f, 'modulation', cmap='autumn', clim=clim, 
			radius=radius,vmin=thresh, under='grey')
		...
		roi_obj.project_sources (s_obj_r, 'modulation', cmap='autumn', clim=clim, 
			radius=radius,vmin=thresh, under='grey')
		...
		roi_obj_l.project_sources (s_obj_l, 'modulation', cmap='autumn', clim=clim, 
			radius=radius,vmin=thresh, under='grey')
		...
		# # =============================================================================
		cb_rep = ColorbarObj(roi_obj_l, cblabel='AUC score', **CBAR_STATE)
