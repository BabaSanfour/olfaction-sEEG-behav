...
for min_sig,win, method in product(min_sigs,wins, methods):
	...
	CBAR_STATE = dict(cbtxtsz=12, txtsz=10., width=.1, cbtxtsh=3.,
	                  rect=(-.3, -2., 1., 4.))
	sc = SceneObj(camera_state=CAM_STATE, bgcolor=(.1, .1, .1),size=(400, 600)) #largeur, hauteur
	# ============================================================================
	...
	for i,freq in enumerate(freqs):
		...
		for elec in range(da.shape[0]):
			...
			time = s_time[idx]
			time_max = np.hstack((time_max, np.mean(time))) if np.size(time_max) else np.mean(time)
		data = time_max
		...
		# # =============================================================================
		cb_rep = ColorbarObj(s_obj_r2, cblabel=feat, border=False, **CBAR_STATE, ndigits=0)
