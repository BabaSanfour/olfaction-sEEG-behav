...
f_form_save = join(path_to_save, '{}_sources_intime_min{}_'+feat+'_{}_th{}_corr.png') 
###############################################################################
...
for win, freq, min_sig, method in product(wins,freqs,min_sigs,methods):
	...
	CBAR_STATE = dict(cbtxtsz=12, txtsz=10., width=.1, cbtxtsh=3.,
	                  rect=(-.3, -2., 1., 4.))
	sc = SceneObj(camera_state=CAM_STATE, bgcolor=(.1, .1, .1),size=(500, 900)) #largeur, hauteur
	# =============================================================================from visbrain import Brain, Colorbar
	for i,t in enumerate(np.arange(0,da.shape[1],step)): #loop across time for each time point
		mask = []
		for elec in range(da.shape[0]):
			da_t = round(da[elec,t],2)
			if da_t > max(perm_thr):
				mask.append(False)
			else :
				mask.append(True)
		da_plot, pow_plot = da[:,t], power[:,t]
		time, data = round(times[t],2), pow_plot
		...
		sc.add_to_subplot(b_obj_top, row=i, col=0, rotate='bottom',
							title=freq+' - win'+str(win)+' th'+th+' Time '+str(time)+'s')
		# #Define a source object with a different color by subject
		xyz = arch_sig['s_xyz']
		elecs_labels_sig=None
		s_obj_c = SourceObj('modulation', xyz, radius_min=10.,radius_max=11., mask=mask)
	...
	sc.screenshot(f_form_save.format(method,min_sig,freq,th),autocrop=True,print_size=(5,12))
