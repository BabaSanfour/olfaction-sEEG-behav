...
f_form_save = join(path_to_save, '{}_signif_min{}_'+feat+'_bsl_{}_{}_{}_th{}_'+conds[0]+'_'+conds[1]+'.png')
###############################################################################
...
win = 1
min_sigs = [1]
methods = ['s_Mai_RL']#'aal','aal_RL',BA
clim_min, clim_max = -80,80
cmap = 'jet'

for bsl, min_sig, method in product(bsls,min_sigs, methods):
	# =============================================================================
	...
	CBAR_STATE = dict(cbtxtsz=12, txtsz=10., width=.1, cbtxtsh=3.,
	                  rect=(-.3, -2., 1., 4.))
	sc = SceneObj(camera_state=CAM_STATE, bgcolor=(.1, .1, .1),size=(500, 400))#largeur, hauteur
	# ============================================================================
	for i,freq in enumerate(freqs):
		...
		b_obj_top = BrainObj('B2', translucent=True, hemisphere='left')
		b_obj_top.alpha = 0.09
		sc.add_to_subplot(b_obj_top, row=i, col=0, rotate='left',
						title=freq[2:]+' win '+str(win)+' th'+th+' - Left & Right Hem')
		...
		s_obj_c = SourceObj('modulation', xyz_sig, symbol='disc',radius_min=10.,radius_max=11.,alpha=.95)
		s_obj_c.set_visible_sources('left')
		...
		b_obj_r.alpha = 0.09
		sc.add_to_subplot(b_obj_r, row=i, col=1, rotate='right')
		s_obj_r = SourceObj('S_right', xyz_sig, symbol='disc', radius_min=10., 
		...
		# # =============================================================================
		cb_rep = ColorbarObj(s_obj_r, cblabel=feat, border=False, **CBAR_STATE)
		sc.add_to_subplot(cb_rep, row=i, col=2, width_max=200, height_max=600)

sc.screenshot(f_form_save.format(method, min_sig,bsl,freqs[0],freqs[1],th),autocrop=True,print_size=(9,5))
