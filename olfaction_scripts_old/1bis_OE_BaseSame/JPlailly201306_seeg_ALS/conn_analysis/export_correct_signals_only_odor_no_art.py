...
from export_inband_amplitude2_noart import return_electrode_names_and_coords,return_non_art,return_sess_correct_trigs_and_rest
...
def compute_trial_sigs(correct_sigs,correct_trigs,sess_timings):
    ...
    return np.array(trial_sigs), np.array(trial_trigs)
...
def return_sess_correct_trigs(write_path,sess_correct_trigs,sess_correct_sigs,sess_timings,trial_types):
    ...
    for i,trial_type in enumerate(trial_types):
        ...
        if not os.path.exists(np_trigs_file):
            ...
            trial_sigs, trial_trigs = compute_trial_sigs(sess_correct_sigs,sess_correct_trigs[i],sess_timings)

            print trial_sigs.shape

            np.save(np_trigs_file,trial_sigs)
            np.save(np_trigs_file.replace('trigs.npy','timings.npy'),trial_trigs)
        else:
...
def compute_subj_correct_sigs(export_dir, name = 'SEMC',exp = 'E',thr_nb_artefact = 120):
    ...
    for sess_index in sess_indexes:
        ...
        print 'sess time', sess_timings.shape
    ...
    for i,sess_index in enumerate(sess_indexes):
        ...
        sess_correct_trigs,nb_removed_trigs_by_trial = return_sess_correct_trigs_and_rest(run,list_sess_timings[i],list_keep_all_non_art[i],trial_types)
...
def test_correct_sigs():
    ...
    compute_subj_correct_sigs(base_dir, name = 'CHAF', exp = exp)
