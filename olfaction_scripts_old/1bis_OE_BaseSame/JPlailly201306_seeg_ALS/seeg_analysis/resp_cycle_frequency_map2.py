...
def estimate_inspi_ratio():
    colors = {'CHAF':'darkblue', 'FERJ':'deepskyblue', 'LEFC':'mediumspringgreen', 
        'SEMC':'yellow','PIRJ':'darkorange', 'VACJ':'red'}
    su_codes = {'CHAF':'P1', 'FERJ':'P2', 'LEFC':'P3','SEMC':'P4','PIRJ':'P5','VACJ':'P6'}
    ...
    for subject in session.query(Subject):
        ...
        if su == 'MICP':
	        ...
	        for run in subject.runs.filter_by(exp = 'R'):
	            ...
	            su_ratio.append(ratio)
	        su_ratio = np.concatenate(su_ratio, axis=0)
	        all_ratio = np.concatenate((all_ratio,su_ratio), axis=0)
	        #balanced histogram as a function of number of trials
	        y,x  = np.histogram(su_ratio, bins = np.arange(0,1,0.01))
	        y_bal = (y.astype(np.float)*100)/su_ratio.shape[0]
	        ax = sns.kdeplot(np.array(su_ratio), bw=0.2,color=colors[su])
	        #ax = sns.distplot(su_ratio,bins=np.arange(0,1,0.01),hist=False,
			#					color=colors[su],norm_hist=True)
	        ax.set_xlim(0,1)
	        #y = (y.astype(np.float)*100)/su_ratio.shape[0]
	        #ax.plot(x[:-1], y, color=colors[su])
	        #find and annot max of frequency rate
	        xmax = x[np.argmax(y)]
	        print('xmax',xmax)
	        ymax = y.max()
	        ax.plot(0,0,'o',color=colors[su],label=su_codes[su])
	        print(subject.name,round(1/xmax,2))
	        print('mean',subject.name,round(1/np.mean(su_ratio),2))
	        ax.legend(loc=0,handletextpad=0.1, frameon=False)
    ax.axvline(np.mean(all_ratio), ls = '-', color='black')
    ax.axvline(np.median(all_ratio), ls = '--', color='black')
    fig.savefig('respi_spectre_R.png')
    fig.savefig('respi_spectre_R.pdf')
